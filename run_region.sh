...
while read line
do
    read chrom chrom_size remain <<<${line}
    chrom_start=1
    while true
    do
        chrom_end=$((chrom_start + size_limit - 1))
        if [ ${chrom_end} -gt ${chrom_size} ]
        then
            chrom_end=${chrom_size}
        fi
        region=${chrom}:${chrom_start}-${chrom_end}
        vg_graphs+=(${output_dir}/${genome_stem}.${region}.bisulfite.vg)

        # Construct graph
        systemd-run --scope --user \
            --property MemoryMax=50G \
            ./vg construct --progress \
                --region ${region} \
                --reference ${output_dir}/${genome_stem}.bisulfite.fa \
                --vcf ${output_dir}/${genome_stem}.bisulfite.vcf.gz \
                > ${output_dir}/${genome_stem}.${region}.bisulfite.vg

        if [ ${chrom_end} -eq ${chrom_size} ]
        then
            break
        fi
...
# for vg_graph in ${vg_graphs[@]}; do
#   vg prune --restore-paths ${vg_graph} > ${vg_graph%.*}.pruned.vg
